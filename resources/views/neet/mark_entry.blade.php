...
<div class="container">
    <form method="POST" id="form-error" action="{{ route('neet-mark-store') }}">
        {{csrf_field()}}
            <div class="header-title margin-bt-2">
                    <div class="header" style="border-bottom: none;">NEET MARK ENTRIES</div>
            </div>   
            ...
            <input type="text" name="chemistry" class="mt-3 form-control"  placeholder="Enter Mark" required value="{{old('chemistry')}}"><br>
              ...
              <label>Biology Mark:</label>
            <input type="text" name="bio" class="mt-3 form-control"  placeholder="Enter Mark" required value="{{old('bio')}}"><br>
    ...
    $(document).ready(function(){
      $('#form-error').submit(function(){
        ...
        var exam_id = $('#exam_id').val();   
         ...
         console.log(number); 
         $.ajax({
           url: "{{url('neet_mobile_validation')}}/"+number+"/"+exam_id,
           type:'GET',
